...
//Store up to k indexes of zero within the window [l, h] so that we know where
//to move l next when the window contains more than k zero.
...
//In that case we can use BigInteger for all indexes. For simplicity, here we
//will use int
//Time: O(n) Space: O(k)
 public int findMaxConsecutiveOnes(int[] nums) {
