...
public ListNode mergeKLists(ListNode[] lists) {
    ...
    return mergeKListHelper(lists, start, end);
}

//Divide - remmber 106B print the file lines in reverse direction
private ListNode mergeKListHelper(ListNode[] lists, int start, int end) {
    if(start < end) {
        int mid = start + (end - start)/2;
        ListNode upper = mergeKListHelper(lists, start, mid);
        ListNode lower = mergeKListHelper(lists, mid+1, end);
...
public ListNode mergeKLists(ListNode[] lists) {
    ...
    //PQ : to keep track of all heads in the list
    PriorityQueue<ListNode> pq = new PriorityQueue<> (lists.length ,(l1, l2) -> { return l1.val - l2.val; } ); //Head tracker
    for(ListNode l : lists) {
        if(l != null)  pq.add(l);
    }
    ListNode tH = new ListNode(42);
    ListNode tmp = tH;
    while(!pq.isEmpty()) {
        tmp.next = pq.poll();
        tmp = tmp.next;
        if(tmp.next != null) pq.add(tmp.next); //Add next heads to the list
    }
    return tH.next;
}
