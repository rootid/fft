...
//I also mentioned explicitly that only two such numbers occur in all cases (except for the element which we want to
//find
...
//leftFirst: One of the two conditions to go left is : if it is the second occurrence of that digit(3 here) than mid
//should be even (even number of elements before it, because everything repeats twice)
//
//1     1    2    3    3    4    4    5    5
//                    mid
//leftSecond: Another condition to go left is : if it is the first occurrence of that digit(4 here) than mid should be
//odd (odd number of elements before it, because everything repeats twice) :
...
//Eventually you'll be looking at the subarray [1,3], with low = 0, high = 1 which implies mid = 0. I think your array
//access nums[mid-1] would throw index out of bounds here. you should check for valid index, or did i miss something?
//is a bug, needs check
//
//return nums[low];
//Why? Shouldn't return nums[mid] be the one finding the right number? Once you exited the loop, the search was
//unsuccessful, you could return fail or throw. are you simply returning "something" in this case or is this actually
//needed for the answer to be correct?
//when the first or last element is the answer, the search will terminate unsuccessfully, but at that point, since we
//know the array has 1 element that matches the condition, we just return it.
