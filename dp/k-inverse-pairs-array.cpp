...
//We must minus dp[i][j-i]. (In fact it minusdp[i-1][j-i], because everyj >= iin dp vecor,it minus
//dp[i-1][j-i]individually)
//For example, if i = 5
//
...
//
//Looking at a potential recursion for dp[n+1][k], depending on where we put the element (n+1) in our permutation, we
//may add 0, 1, 2, ..., n new inverse pairs. For example, if we have some permutation of 1...4, then:
//
...
//
//In the code, we refer to -ds[n][k-n+1] instead of -ds[n][k-n] because the n being considered is actually n+1. For
//example, when n=2, we are appending information about ds[2][k] to new, so our formula of dp[n+1][k] = ds[n][k+1] -
//ds[n][k-n] is dp[2][k] = ds[1][k+1] - ds[1][k-1].
