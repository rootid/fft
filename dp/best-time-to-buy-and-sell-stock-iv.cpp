...
//Suppose we are facing two segments (v1,p1) and (v2, p2) under the limit v1 <
//p1 < v2 < p2,
//we would have two possible operations:
//1. buy at v1, sell at p1; buy at v2, sell at p2 ( p1 - v1 + p2 - v2 )
//2. buy at v1, sell at p2 ( p2 - v1 )
//What we need to do is to find out which could make more profit. We would meet
//these situations:
//prices[v2] <= prices[v1]
//it’s obvious that we’d better make two transactions: p1-v1 and p2-v2, and v2
//is more possible to make bigger profit. So, we pop out (v1,p1) from the
//stack.
//prices[v2] > prices[v1]
...
//stack.
//prices[p2] >= prices[p1] We are not sure about using one or two transactions
//to get higher profit, so we push two possible results into the profits array.
//Here comes the key point: we push p2-v1 and p1-v2 into profits array. Why? If
//we could only use one transaction, p2-v1 is what we want, and if with two
