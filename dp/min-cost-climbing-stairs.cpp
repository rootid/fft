...
//######################################### DP -2D #########################################
//Simple DP solution - at each step we make a decision to take this step or not. If we take current step, then we take
//the min of taking and not taking previous step and add the current cost.
//If we don't take the current step, then we have to take the previous step - so directly get the value from taking the
//previous step
...
//######################################### DP #########################################
//There is a clear recursion available: the final cost f[i] to climb the staircase from some step i is f[i] = cost[i] +
//min(f[i+1], f[i+2]). This motivates dynamic programming.
//Algorithm
//Let's evaluate f backwards in order. That way, when we are deciding what f[i] will be, we've already figured out
//f[i+1] and f[i+2].
//We can do even better than that. At the i-th step, let f1, f2 be the old value of f[i+1], f[i+2], and update them to
//be the new values f[i], f[i+1]. We keep these updated as we iterate through i backwards. At the end, we want min(f1,
//f2).
