...
//########################################## DFS ########################################## 
//Player1 can win if and only if they can make a move after which player2 will lose no matter how they move. 
//Main Idea : So we can check all possible moves of player1 and call the same function to see if player2 will lose after this move. 
//I use caching to memorize states for each possible desiredTotal and set of chosable integers. Since maxChoosableInteger is not greater than 20, all currently possible moves can be represented as a bit flag.
private :
