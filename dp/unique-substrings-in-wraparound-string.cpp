...
//O(n^2 logn), Each valid substr can be represented by the first char and the
//length, instead of the whole string.
int findSubstringInWraproundString(string p) {
    int n = p.size();
...
//ACCEPTED
//O(n^2). For substrs starting at the same char, we only need to record the
//longest one. Because it covers all the shorter substrs starting from the
//char. The length is the number of substrings starting at the char.
int findSubstringInWraproundString(string p) {
...
//O(n). Getting the longest substr starting from each char can be done in
//linear time. We can use two pointers to keep track of the current valid
//substring.
//
//1. A non-empty substring sub of s is uniquely defined by the initial char
//sub[0] and length sub.size().
//2. If string sub is a non-empty substring of s, then all its prefix
//sub.substr(0, L) must also be unique substrings of s, where 1 <= L <=
//sub.size().
int findSubstringInWraproundString(string p) {
