...
//Can we improve on the ideas in Approach #1? When a row is filled with XXX 1s, we do O(X2)O(X^2)O(X​2​​) work to enumerate every pair of 1s. This is okay when XXX is small, but expensive when XXX is big.
...
//Let's call a row to be heavy if it has more than N\sqrt N√​N​​​ points. The above algorithm changes the complexity of counting a heavy row from O(C2)O(C^2)O(C​2​​) to O(N)O(N)O(N), and there are at most N\sqrt N√​N​​​ heavy rows.
//
//
//Time Complexity: O(NN)O(N \sqrt N) where NNN is the number of ones in the grid.
