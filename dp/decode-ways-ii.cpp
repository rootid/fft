...
//You can read it into "A" and "A" to "A" and "I" or you can treat it as a two digit number as "K" to "S". That is the way you decode the code.
//So if you treat the current character s.charAt(i) as a single character, the dp result of the index i should be the number of
...
//That is basic thinking of dp for this problem. Then what should be done is to find what kind of pairs or single numbers are valid.
//Like a single "0" is not valid, and there is no answer for something like "30".
...
//1. If c == '*', then the number of ways to finish in total is: we could put * as a single digit number (9*e0), or we could pair * as a 2 digit number 1* in 9*e1 ways, or we could pair * as a 2 digit number 2* in 6*e2 ways. The number of ways to finish with an open 1 (or 2) is just e0.
//2. If c != '*', then the number of ways to finish in total is: we could put c as a single digit if it is not zero ((c>'0')*e0), or we could pair c with our open 1, or we could pair c with our open 2 if it is 6 or less ((c<='6')*e2). The number of ways to finish with an open 1 (or 2) is e0 iff c == '1' (or c == '2').
//
