...
//If the root doesn't have left subtree or right subtree, then the root itself is left boundary or right boundary. Note
//this definition only applies to the input binary tree, and not applies to any subtrees.
...
//PreOrder on the left half tree to fetch left Border elements, InOrder on the left and right half tree to fetch leaf
//elements PostOrder on the right half tree to fetch right Border elements
