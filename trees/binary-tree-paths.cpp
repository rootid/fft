...
//as each time you find a valid path, you need to copy the array first before storing it in the result.
//Even if you use StringBuilder, toString creates a copy of the wrapped char array and is not an O(1) operation.
...
//So the time complexity of your solution is still O(n^2) --- max_length_of_each_path = O(n), 
//max_number_of_leaves = O(n). This is a very loose bound and the actual time complexity may be smaller, but not really O(n)
