...
//1. keep track of the previous node to identify when the current node's value is different from previous node's value.
//2. When the values of both nodes are different, update max frequency if occurrence of current value is greater.
