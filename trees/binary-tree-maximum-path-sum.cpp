...
//############################### MathPathSumHelper  ###############################
//Idea :
//traversal every nodes as the top of sub tree and calculate left max and right max individually, then keep update max.
//T
//A path, in this problem, refers to any sequence of nodes from some starting node to any node in the tree along the
//parent-child connections.
//In the example, you start at either the 2 or 3, then traverse to its parent, the 1 at the root, then finish at the
//root's other child.
...
//But to the //upper layer(after return statement), we cannot choose both left and right branches, so we need to select
//the larger one, so we use max(left, right) + node.val to prune the lower branch.
...
//Not many problems exhibit such a clear sub-structure. Here we need to simply solve the problem for a tree with up to
//two levels and then recursively repeat that logic.
...
//Only greater of the first three values can move up the ladder. Repeat this at every step, and we have a bottom up
//solution. More comments with C implementation given below
...
def maxPathSum(self, root):
    ...
    maxend(root)
    return self.max
//Here the helper is similar, but only returns the first of the two values (the max sum of all paths ending in the given
//node). Instead of returning the second value (the max sum of all paths anywhere in tree rooted at the given node), it
//updates a "global" maximum
