...
//######################################### Backtrack ######################################### 
//backtracking solution using recursion with focus on simple readability, which was inspired by this Java solution. Base case is when the index equals the size of the string. This occurs when the entire string has been completely processed. When the base case is reached, if the count of remaining open parens without a corresponding close paren is equal to 0, then return true, otherwise return false. The ongoing count of open parens is continually processed for each index of the string from left-to-right. Whenever an open paren ( is found, the count of open parens is incremented by 1. Likewise, whenever a close paren ) is found, the count of open parens in decremented by 1. If the ongoing count of open parens is less than 0, then we know there are too many close parens, and return false immediately. Whenever a * occurs in the string, this is processed by checking each of 3 possibilities:
//
//the * as a ( ==> this is processed by incrementing the ongoing open count by 1: open+1
//the * as a ) ==> this is processed by decrementing the ongoing open count by 1: open-1
//the * as a ""(empty string) ==> this is processed by keeping the ongoing count of open "as is" without changing the value
