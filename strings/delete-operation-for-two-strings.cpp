...
//int n = word2.size();
//DP array dp[m + 1][n + 1]
//dp[i][j] means how many operations we need to make word1[0 - i] and
//word2[0-j] equal.
//Initialization:
//dp[0][j] = 0,1,2,3...n
//dp[i][0] = 0,1,2,3...m
...
//2. Skip character in the first string (dp[i - 1][j]). Add one deleted
//character
//3. Skip character in the second string (dp[i][j - 1]). Add one deleted one
//character
...
//########################################### Pytonic  ###########################################
//Let dp(i, j) be the answer for strings A[i:] and B[j:]. Let's try to compute
//it by a top-down dp:
//
//When i == len(A) or j == len(B), one of the strings is empty, so the answer
//is just the sum of the remaining lengths.
//When A[i] == B[j], the answer is just dp(i+1, j+1). For example, when
//evaluating the distance between "acai" and "apple", we only need to look at
//the distance between "cai" and "pple".
//When A[i] != B[j], then they both cannot be in the final word, so we either
//delete A[i] or B[j]. Thus, our answer is 1 + min(dp(i+1, j), dp(i, j+1)).
