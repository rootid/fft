...
//of same-character contiguous blocks within the string. For example, if s =
//"110001111000000", then groups = [2, 3, 4, 6].
...
//never be larger.
//Algorithm
//Let's create groups as defined above. The first element of s belongs in it's
//own group. From then on, each element either doesn't match the previous
//element, so that it starts a new group of size 1; or it does match, so that
//the size of the most recent group increases by 1.
...
//We can amend our Approach #1 to calculate the answer on the fly. Instead of
//storing groups, we will remember only prev = groups[-2] and cur = groups[-1].
//Then, the answer is the sum of min(prev, cur) over each different final
//(prev, cur) we see.
//

//Time Complexity: O(N), where NNN is the length of s. Every loop is through
//O(N)O(N)O(N) items with O(1)O(1)O(1) work inside the for-block.
