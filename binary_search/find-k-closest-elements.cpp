...
def findClosestElements(self, arr, k, x):
    ...
    return list(res)
//######################################### binary search #########################################
//It's the first index i so that arr[i] is better than arr[i+k] (with "better"
//meaning closer to or equally close to x).
//Then I just return the k elements starting ther
//binary-searching for x and then expanding to the left and to the right like
//I've seen in other binary search solutions.
//The binary search costs O(log n) (actually also just O(log (n-k)) and the
//subList call probably only takes O(1). As @sagimann pointed out, subList
//returns a view, not a separate copy. So it should only take O(1). Can be seen
//for example in ArrayList's subList and the SubList constructor it calls. I
//also checked LinkedList, it gets its subList method inherited from
//AbstractList, where it also takes only O(1). And AbstractList is a basis for
//most lists.
//Edit: I also implemented it in Go now, to make it O(log n). Edit 2: Ha,
//didn't have to do that, as the Java version apparently already was O(log n)
//(I didn't originally know Java returns a view, only added that now). Edit 3:
//Lol, I had mistakenly written "Python" in the title instead of "Ruby" but
//apparently nobody noticed (and it's at 1800 views). Fixed that now
...
//arr. Then, we determine the indices of the start and the end of a subarray in
//arr, where the subarray is our result. The time complexity is O(logn + k).
//In the following code, arr[index] is the first number which is euqal to or
//geater than x (if all numbers are less than x, index is arr.size()), and the
//result is arr[i+1, i+2, ... j]
//######################################### JAVA version: #########################################
public List<Integer> findClosestElements(List<Integer> arr, int k, int x) {
