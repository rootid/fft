...
//Algorithm
//
//In order to reverse the subsections of the minminmin array, as discussed in
//the last approach, we can also start by initializing the resultant
//arrangement resresres with the minminmin array i.e. by filling with elements
//(1,n)(1,n)(1,n) in ascending order. Then, while traversing the pattern sss,
//we can keep a track of the starting and ending indices in resresres
//corresponding to the D's in the pattern sss, and reverse the portions of the
//sub-arrays in resresres corresponding to these indices. The reasoning behind
//this remains the same as discussed in the last approach.
public class Solution {
    public int[] findPermutation(String s) {
        int[] res = new int[s.length() + 1];
...
//
////######################################### pytonic #########################################
//My 1-liner tells sorted that the (larger) number i comes before the (smaller)
//number j iff they're both under the same D-streak, i.e., iff there's no I
//between them. (I'm not totally sure that i will always be the larger number,
