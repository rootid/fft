...
//Snake
//Let's have some fun with 2D Matrices! Write a method findSpiral to traverse a 2D matrix of 
//ints in a clockwise spiral order and append the elements to an output ArrayList if Integers.
//Example:
//Input Matrix :      
//    {1, 2, 3}
//    {4, 5, 6}
  ...
  public static ArrayList<Integer> findSpiral(int[][] arr) {
    ...
    while(top <= bottom && left <= right) {
      ...
      for(int i=left; i<=right;i++,cnt++) 
        resultList.add(arr[top][i]);
      top++;

      for(int i=top;i<=bottom;i++,cnt++) 
        resultList.add(arr[i][right]);
      right--;
