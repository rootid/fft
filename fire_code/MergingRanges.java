...
public class MergingRanges {
  ...
  public static ArrayList<Interval> mergeIntervals(ArrayList<Interval> intervalsList) {
      if(intervalsList == null) throw new IllegalArgumentException("Please enter valid interval List");
      ...
      for(int i=0; i< intervalsList.size(); i++) {
          if(resultList.size() == 0 || resultList.get(resultList.size() - 1).end - intervalsList.get(i).start < 0) {
              resultList.add(new Interval(intervalsList.get(i).start, intervalsList.get(i).end));
          } else {
              resultList.get(resultList.size() - 1).end = Math.max( intervalsList.get(i).end , 
