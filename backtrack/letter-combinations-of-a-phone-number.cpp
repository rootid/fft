...
//    return R;
//}
//I have a different approach and it is more intuitive to me. This is similar
//to printing all numbers with k digits. For example k=3, you start at 000,
//then you keep coming up: 001, 002, 003, ... until you reach 009, then reset
//last bit, and increase the next bit to 010, then 011, 012, etc.
//So we can solve this problem in a similar way. If our input string is "23".
//Because 2 = "abc", 3 = "def", we can start from the smallest "ad", then go
//up: "ae", "af", then we have to reset last char from "f" to "d" and we have
//"bd", "be", "bf", etc. This way, we don't need backtracking, recursive, or
//List.
//Hope this helps for some people.
//vector<string> letterCombinations(string digits) {
//    vector<string> R;
...
//    return R;
//}
//I've decided to experiment with different approach than backtracking and
//recursion. It reminds a bit radix-sort style. The main idea is to keep a
//buffer of chars representing the word. At first it is initiated by first
//letter from the corresponding mapping list for each digit. Then we
//sequentially try all letters of a first digit. Then "increment" letter for
//the second digit, and run through all first ones again. Etc. Like spinning
//the rings of a number lock.
...
//    }
//}
//Actually, this approach looks more effective than recursive search not only
//because it's iterative but also because we reuse the tail of our buffer
//making some kind of memoization.
//Have a nice one!
