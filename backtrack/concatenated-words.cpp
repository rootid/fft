...
//######################################### DP #########################################
//TC : O(N^3)
//For any qualified word, there must be at least 3 indexes (at least 1 besides
//0 and n-1 which n is the length of the word), which can be used to split the
//whole string to at least two sub strings and all sub strings can be found in
//words.
//E.g. input ["cat","cats", "dog", "sdog","dogcatsdog"], for word dogcatsdog,
//there are 2 sets of numbers: [0, 3, 6, 10] and [0, 3, 7, 10] which can be
//formed by concatenating [dog, cat, sdog] and [dog, cats, dog] respectively.
//So, we can use a vector<int> dp(n+1) to store if w.substr(0, i) can be formed
//by existing words. Once i reach to n and it is not the word itself, we put
//the word to results.
...
//problem is just one more step further.
...
//than it.
//So we can first sort the input by length of each word, and only try to form
...
//Consider the word as a topologically sorted directed graph, where each node
//is a letter,
//and an edge exists from i to j if word[i:j] is in our wordlist, [and there is
//no edge from i=0 to j=len(word)-1].
