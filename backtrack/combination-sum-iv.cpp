...
// 
// 3 - 1 - 0
//target is the sum of numbers in the array. Imagine we only need one more number to reach target, this number can be any one in the array, right? So the # of combinations of target, comb[target] = sum(comb[target - nums[i]]), where 0 <= i < nums.length, and target >= nums[i].
...
//implementation of bottom-up approach, you will notice that it may have more "intermediate steps" than the top-down one.
