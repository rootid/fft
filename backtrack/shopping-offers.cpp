...
//You are given the each item'sIdx price, a set of special offers, and the number we need to buy for each item. The job is to output the lowest price you have to pay for exactly certain items as given, where you could make optimal use of the special offers.
...
//Greedy doesn't work. 
//One example is [3, 2], [[1, 2, 4] [2, 1, 5]], [6, 6]. We need 2 first offers and 2 second offers. Greedy would choose 3 first offers.
class Solution {
public:
    int shoppingOffers(vector<int>& price, vector<vector<int>>& special, vector<int>& needs) {
...
//3. This code have O(special offers size) time complex. When the input is small, it's not the best time complex. And it also not very general.
int shoppingOffers(vector<int>& price, vector<vector<int>>& special, vector<int>& needs)
{
	int n = price.size();
	for (int i = n; i < 6; i++)
...
//Our goal is to minimize the sum of our discount. Our top-down dp function, discount, will do this. 
//Suppose we are currently considering the ith special offer, and we have some order. 
//We must use some legal number of this offer special[i]: if we use K of them, then we will get a discount of K * value[i], and in turn this will specify the candidates for our dp.
