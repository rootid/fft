...
//######################################### Group by Category  #########################################
//Time Complexity: O(M∗N). (Recall in the problem statement that M,NM, NM,N are the number of rows
...
//This leads to the following idea: remember the value of that diagonal as groups[r-c]. If we see a
...
//Every element belongs to some diagonal, and it's previous element (if it exists) is it's top-left
//neighbor. Thus, for the square (r, c), we only need to check r == 0 OR c == 0 OR matrix[r-1][c-1]
//== matrix[r][c]
class Solution {
    public boolean isToeplitzMatrix(int[][] matrix) {
        for (int r = 0; r < matrix.length; ++r)
