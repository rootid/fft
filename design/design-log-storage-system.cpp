...
//##################################### Use of dictionary   #####################################
//same idea of truncating/pruning the timestamp as per granularity, but converted the timestamp to a tuple by splitting
//at : before storing/comparing so thinking of which index to prune the tuple as it was simple to think of. (For Year,
//prune the timestamp tuple from index 1 and onwards, for Month prune from index 2 and onwards...).
//lexicographic comparison of elements in the tuple will work as intended because year/month/... are all padded to the
//same length in the given input. And since each of year/month/day, etc have the same length we could even directly
//slice the string without converting to tuple which brings us to back to your solution.
