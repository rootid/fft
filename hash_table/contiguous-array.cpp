...
//We can consider size as the zero point, so the array indexes will be [0... 2
//* size].
...
//if two prefix subarrays A and C of nums have the same 2 * sum - len, we can
//get len(B) = len(C) - len(A)
int findMaxLength(vector<int>& nums) {
    int sum = 0, max_len = 0;
...
//index i and j. Also put the sum to index mapping to a HashMap to make search
//faster.
