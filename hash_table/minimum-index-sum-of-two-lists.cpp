...
//The idea is to use sum of index in both list as score for every restaurant
//ever appeared, but need to manage punishment for not show up in either list.
//
//Use sum of size as a punishment for missing in either list : l1.size() +
//l2.size();
//Create a map, for every restaurant appear in either list, initialize the
//score as 2 times punish;
//For every restaurant in every list, remove 1*punish for showing up in 1 list.
//On top of the punishment, don't forget to add indexs as score, the smaller
//the better.
...
//            rank[p.second].push_back(p.first);
//        }
//std::map::[key] is an overloaded operator act just the same as
//java.util.Map.get(key), retrieve the value for the given key.
//The [] will create an default value if the key is not found, in this case
//rank[p.second] will create an empty vector if p.second is not found as a key
//in the map rank.
//
//equals:
//
...
//For every index, value pair (j, v) in B, we have some candidate
//sum-of-indexes i + j, where i = Aindex[v] if it exists.
//If the candidate sum is better, it becomes our new answer; if the candidate
//sums are the same, then we append to our answer.
