...
//In a more general setting, we use the bit lengths of both n and start & -start (the lowest bit of start) to compute the mask which represents 232−mask2^{32 - \text{mask}}2​32−mask​​ ip addresses. Then, we adjust start and n appropriately.
//In Java and C++, we should be careful to use long data types to represent the converted ip addresses, since the number could exceed 2312^{31}2​31​​.
...
def ipToCIDR(self, ip, n):
    ...
    return result
'''
//To make it more clear, my answer for n = 8 is ['255.0.0.7/32', '255.0.0.8/30', '255.0.0.12/31', '255.0.0.14/32'].
//As we start from 255.0.0.7 with 32 bits fixed, then we still need to find 7 ips, which indicates 7 = 4 + 2 + 1 (30, 31, 32). For example, starting from 255.0.0.8, we want generate 4 ips which has the same 30 bits prifix as 255.0.0.8. I mean they look like this 11111111 00000000 00000000 000010xx. Same rules for the others.
